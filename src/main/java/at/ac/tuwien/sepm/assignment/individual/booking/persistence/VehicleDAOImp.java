...
public class VehicleDAOImp implements VehicleDAO {
    ...
    private String getSqlQueryForFilter(SearchFilter searchFilter){
        ...
        } else if (searchFilter.getMinPrice() != null && searchFilter.getMaxPrice() != null && searchFilter.getMinPrice() == 0 &&  searchFilter.getMaxPrice() != 999){
            querys.add(" basePrice <= " + searchFilter.getMaxPrice());
        } else if (searchFilter.getMinPrice() != null && searchFilter.getMaxPrice() != null && searchFilter.getMinPrice() != 0 && searchFilter.getMaxPrice() == 999){
            querys.add("basePrice >= " + searchFilter.getMinPrice());
        }
        if (searchFilter.getDriverLicense() != null && !searchFilter.getDriverLicense().equals("")){
        ...
        if (searchFilter.getStartDate() != null && searchFilter.getEndDate() != null){
            ...
            tmp += " vid NOT IN (SELECT vid FROM BookingVehicle LEFT JOIN Booking ON BookingVehicle.bid = Booking.bid WHERE " +
                "(beginnDate <= '" + searchFilter.getStartDate() + "' AND endDate >= '" + searchFilter.getStartDate() + "') AND status = 'OPEN')" ;
            querys.add(tmp);
        } else if (searchFilter.getStartDate() == null && searchFilter.getEndDate() != null){
            tmp += " vid NOT IN (SELECT vid FROM BookingVehicle LEFT JOIN Booking ON BookingVehicle.bid = Booking.bid WHERE " +
                "(beginnDate <= '" + searchFilter.getEndDate() + "' AND endDate >= '" + searchFilter.getEndDate() + "') AND status = 'OPEN')" ;
            querys.add(tmp);
        }
        if (!querys.isEmpty()){
            for (String s : querys) {
